...
```bash
# 檢查 Figma token
.venv/bin/python -c "import os; token=os.environ.get('FIGMA_ACCESS_TOKEN',''); print(f'Token: {\"found (\" + token[:8] + \"...)\"}' if token else 'NOT SET'}')"
```
...
```bash
.venv/bin/python -c "from PIL import Image; import numpy; print('ok')"
```
...
```bash
.venv/bin/python scripts/figma-export.py "<figma_url>" --output design.png
```
...
```bash
.venv/bin/python scripts/pixel-diff.py design.png screenshot.png --output diff.png --threshold 5 > stats.json
```
...
```bash
.venv/bin/python scripts/generate-report.py \
  --design design.png \
